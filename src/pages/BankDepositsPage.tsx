...
function AddDepositPanel({ onClose, onSaved, banks }: { onClose: () => void; onSaved: () => void; banks: Bank[] }) {
    const today = new Date().toISOString().split('T')[0]
    const defaultBank = banks.find(b => b.is_default) || banks[0]
    const [form, setForm] = useState({
        depositDate: today, bankName: defaultBank?.bank_name || '', accountNumber: '',
    ...
    const handleSave = async () => {
        ...
        if (!form.bankName) { setError('Please select a bank'); return }
...
export default function BankDepositsPage() {
    ...
    const filtered = deposits.filter(d => {
        ...
        const matchesBank = !selectedBank || d.bank_name === selectedBank
